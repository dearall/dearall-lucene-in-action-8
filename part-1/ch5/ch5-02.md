...
- **search(Query query, int n, Sort sort)** 使用指定的排序 Sort 实现对匹配结果排序，不对命中结果计算评分。
- **search(Query query, int n, Sort sort, boolean doDocScores)** 使用指定的排序 Sort 实现对匹配结果排序，并控制是否对命中文档计算评分。如果 doDocScores 为 true 则对命中文档计算评分，否则不计算。
