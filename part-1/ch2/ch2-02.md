...
>可以把整个索引想象为一个数据库，但它没有传统的关系型数据库的概念，没有所谓的主键（primary key）关联关系，因此，词典内每条索引项除了要单词本身的一些信息外，还有指向“倒排表”的指针。同理，索引数据库也没有关系型数据库中 table 中的概念，因此，它使用 Postings List 术语表示倒排表的语义。
