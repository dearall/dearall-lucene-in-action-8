...
&emsp;&emsp;说起来好像很复杂，实际用起来也没那么麻烦，Lucene 将理论上的复杂性屏蔽在简单的 API 之下，使我们用这些简单的编程接口，就可以将 FeatureField 特性应用到自己的程序中来。上面代码中：
```
    Query boost = FeatureField.newSaturationQuery("features", "pagerank");
```
基于 'features' 域 'pagerank' 特性创建了一个饱和度查询，把这个查询以 Occur.SHOULD 子句形式加入到布尔查询中，这样就将特性查询和索引中的特性值关联起来，影响最终查询结果。
...
```
选项中 FIELD_TYPE.setTokenized(false) 表明域不进行分词，即将特性名作为一个单一的词元（token）。FIELD_TYPE.setOmitNorms(true) 表明忽略 Norms 信息的存储。FIELD_TYPE.setIndexOptions(IndexOptions.DOCS_AND_FREQS) 表明索引特性名词元，并存储词频。但这里存储的词频不是特性名真正的词频，而是将构造器传进来的特性值（feature value）作为词频存储起来。这是 FeatureField 域最明显的特征，也是它最巧妙的地方，有别于一般的 Field 的设计。
...
这是很长的一大节，因为 Field 子类非常多，以适应不同的查询需求。虽然类型很多，常用的也就是前面介绍的几种。
