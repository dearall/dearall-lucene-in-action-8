...
&emsp;&emsp;**IndexWriter** 类是 Lucene 的顶级 API，位于 lucene-core 模块的 org.apache.lucene.index 包中，用于创建和维护索引（JavaDoc: An IndexWriter creates and maintains an index），Lucene 所有关于索引的复杂结构和算法都隐藏在这个顶级类之下，它是所有索引操作的中心。

&emsp;&emsp;目前，IndexWriter 只有一个构造器：**IndexWriter(Directory d, IndexWriterConfig conf)**，其中 Directory 表示索引目录，是 Lucene 为存储索引文件提供的一个抽象层（JavaDoc: A Directory provides an abstraction layer for storing a list of files）。这个目录只包含文件（没有子目录结构）。它是一个抽象类，Lucene 为其提供了多种种类型的子类，例如 FSDirectory, RAMDirectory, FilterDirectory CompoundDirectory, FileSwitchDirectory 等等，以适应不同的存储环境。其中 RAMDirectory 表示内存中的 Directory 实现，它是测试代码的最佳选择。但由于其低效的同步问题从 Lucene 7.5 版本开始不再鼓励使用，且被 @Deprecated 标注，鼓励由 MMapDirectory 取代。虽然直到 8.11 版本还一直存在，但到 Lucene 9.0 版，RAMDirectory 已被移除。
...
&emsp;&emsp;IndexWriterConfig 中可配置的信息非常丰富，前一节中提到的使用复合文件的配置和段的合并策略配置就是通过该对象的 setUseCompoundFile(boolean useCompoundFile) 和 setMergePolicy(MergePolicy mergePolicy) 方法配置的。Lucene 为 IndexWriterConfig 对象的每个选项都设置了默认的配置，因此创建好 IndexWriterConfig 对象后，不用设置任何选项，就可以直接作为构造器参数，即使用默认配置创建 IndexWriter 对象。
...
&emsp;&emsp;IndexWriter 对索引的改变会缓存在内存中，并且定期刷新（flush）到 Directory 表示的索引文件中（由上述改变操作的方法调用）。刷新操作会在两种条件下触发，一种是从上一次刷新到现在新添加了足够数量的文档时，另外一种情况是由所有新添加的文档占用一定数量的内存时。不论先满足哪一种条件，都会触发刷新操作。刷新操作会在 Directory 表示的索引文件目录内创建新的段(segment)，以及该段所包含的所有文件。对新文档数量触发刷新的配置可通过 IndexWriterConfig.setMaxBufferedDocs(int maxBufferedDocs) 设置一个具体的 maxBufferedDocs 值，当新添加 document 数量超过这个设置值时，触发刷新操作。也可以给这个选项传递 DISABLE_AUTO_FLUSH 常量值，表示不使用添加一定的文档数量来作为触发刷新操作的条件，而且默认值就是 DISABLE_AUTO_FLUSH，所以默认是不启用该条件的。对新添文档占用一定数量的内存触发刷新的配置可以通过 IndexWriterConfig.setRAMBufferSizeMB(double ramBufferSizeMB) 设置具体的缓存大小，默认值是 DEFAULT_RAM_BUFFER_SIZE_MB，即默认缓存刷新条件是16MB。也可以设置该选项为 DISABLE_AUTO_FLUSH 来禁用内存刷新条件。为了加快索引速度，应该使用较大的内存缓存触发刷新。

&emsp;&emsp;注意，刷新操作（flushing）把 IndexWriter 内部缓存的状态转移到索引，此时这些变化对 IndexReader 对象来说还是不可见的，除非 IndexWriter.commit() 或者 IndexWriter.close() 方法被调用，IndexReader 对象才能看到索引中真正的变化。刷新操作可能会触发索引中一个或多个段的合并操作，段合并操作默认情况下在一个后台线程中执行，因此不会阻塞添加文档的调用。

&emsp;&emsp;打开一个 IndexWriter 对象会在 Directoy 表示是目录内创建一个锁文件。试图在同一个目录内打开另一个 IndexWriter 会导致发生 LockObtainFailedException 异常。
...
-  IndexWriter.addDocuments(Iterable<? extends Iterable<? extends IndexableField>> docs) 一次性向索引中添加一个文档集合。
...
&emsp;&emsp;位于 org.apache.lucene.document 包的 **Document** 类实现了 Iterable<IndexableField> 接口，是一个简单的对象，作为域的容器，其中包含一组域(a set of fields)，用于表示一个完整的文档。但它的地位和作用却非常重要，它是 Lucene 索引和搜索的操作单元(JavaDoc: Documents are the unit of indexing and search)，所有的索引数据管理和搜索结果的获取都要使用 Document 对象进行。要说明的是，域(field)是真正携带索引内容的实体，它拥有一个名字和一个文本值。域可以被文档存储，这种情况下，当搜索到该文档时，域值可以被获取回来。因此，每个文档应包含一个或多个要存储的域来唯一识别出该文档。

&emsp;&emsp;Field 类实现了 IndexableField 接口，是域对象的具体实现，它是 Lucene 索引的基础组件和基本元素。Field 是 Document 的一部分（JavaDoc: A field is a section of a Document）。每个域由由三部分组成：名称、类型、值。值可以是文本数据（String, Reader 或预分析的 TokenStream 类型），二进制数据（byte[] 类型），或者数值数据(Number 类型)。Field 并不一定存入索引，它是索引内容的携带者和索引方式控制者，是否存入索引，是可选的。如果存入索引，那么它可以在搜索结果的命中文档中获取回来。Field 类带有一个由 IndexableFieldType 接口表示的域类型，用于控制该 Field 在写入索引时的控制信息。通过这些控制信息，Field 控制着数据在索引中的处理方式，包括是否将其自身存入索引的选项。这个 API 及其相关的组件的内容非常丰富，会在后面不同章节逐步展开。
...
public class IndexingTest {
  ...
  static {
    ...
    cityType.setTokenized(true);
    cityType.setIndexOptions(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS);
    cityType.freeze();
  }
...
② 在 Directory 对象上创建 IndexWriter 对象。这里通过 getWriter() 方法，因为程序在多处使用该方法获取 IndexWriter 对象。

③ 创建 Document 对象，用于盛放接下来的多个 Field 对象。

④ 每个 Field 对象需要一个 IndexableFieldType 类型的参数，用于指定所创建 Field 对象的域类型。实际上它是一个接口类型，由 FieldType 提供具体实现。FieldType 是一个类似于 IndexWriterConfig 的配置对象，用于指定要创建 Field 对象的详细配置信息。FieldType 的可配置信息非常丰富，涉及到 Field 对象的方方面面，后续章节会逐渐展开其细节内容。 

⑤ 将创建好的 Field 对象加入到 Document 对象。

⑥ 将 Document 对象添加到 IndexWriter 执行添加索引操作。

⑦ getWriter() 方法。使用 WhitespaceAnalyzer 分析器和已经准备好的 Directory 对象创建 IndexWriter 实例并返回。

⑧ 虽然是一个工具方法，实际上它是建立了一个简单的 TermQuery 查询来搜索在 setup() 方法中建立好的索引，并返回搜索结果的 hitCount 值，也就是匹配查询字符串的文档总数。

⑨ 验证写入的文档数和索引中存在的文档数相等。

⑩ 验证写入的文档数与 reader 读取索引所获得的文档数相等。
...
&emsp;&emsp;如果需要通过 Term 删除单个文档，必须确保在每个文档中都通过 Field 对象索引了这个域（通过相同的域 name 标识），并且索引中所有 document 的这个域的域值唯一，这样才能将这个文档单独找出来将其删除。这与数据库中表的主键（primary key）具有相同的概念，但在 Lucene 中没有这种强制的方法。可以给这个域设置任何名字（通常用 ID 这个名字），这个域应该被设置为不经分析的索引（通过 FieldType.setTokenized(false) 来禁用对域值文本的分析），以确保其不会被分析器切分成分离的语汇单元。然后利用该域来删除对应的文档，如下：
...
我们看看程序 2.2中 deleteDocuments() 方法的实践。代码中创建了两个测试用例来展示 deleteDocuments() 方法的使用，并演示了删除文档调用 commit() 后的效果。

<table width="100%"><tr><td bgcolor=green><font color=black>Listing 2.2 从文档中删除文档 Deleting documents from an index</td></tr></table>

```

  @Test
  public void testDeleteBeforeCommit() throws IOException {
    IndexWriter writer = getWriter();
  ...
  @Test
  public void testDeleteAfterCommit() throws IOException {
    IndexWriter writer = getWriter();
    assertEquals(2, writer.getDocStats().numDocs);
...
③ 确认 writer 中有删除的文档
④⑤ 索引中文档数量没有发生变化
⑥ 删除文档后，调用 writer.commit() 方法，提交本次对索引的更改
⑦ 确认删除操作被提交，缓存已清空
...
```
  ...
  public void testUpdate() throws IOException {
    ...
    long sequence = writer.updateDocument(new Term("id", "1"), doc); // ②
...
本例中，我们用新文档来替换 id 为 1 的旧文档。通过 updateDocument() 方法更新文档，这样，就完成了索引中文档的更新操作。
