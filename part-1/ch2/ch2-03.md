...
我们看看程序 2.2中 deleteDocuments() 方法的实践。代码中创建了两个测试用例来展示 deleteDocuments() 方法的使用，并演示了删除文档调用 forceMergeDeletes() 后的效果。
...
② 删除 id 值为 1 的文档
③ 确认 writer 中有删除的文档
④⑤ 索引中文档数量没有发生变化，因为默认的合并策略是 TieredMergePolicy，它有很多的条件检测，其中一个就是段的数量不是1，显然，我们只有一个段，因此比删除文档仍然存在于索引段中。
...
```
  ...
  public void testUpdate() throws IOException {
    ...
    long sequence = indexWriter.updateDocument(new Term("id", "1"), doc); // ②
