...
搜索操作和索引操作一样快速和简单，其功能之强大非常惊人，第 3、5、6 章用整整三章篇幅详细阐述其详细内容。现在先来看看 Searcher 这个简单的命令行程序，用它来搜索上一节 Indexer 程序建立的索引库。Searcher 程序的目的是演示 Lucene 搜索 API 的用法，真正的搜索程序采用 web 或带有 GUI 的桌面形式，或者是一个完全的 web 应用程序，等等。
...
Searcher 程序，最初由 Erik 为在 java.net 上所写的 Lucene 介绍性文章所编写，作为 Indexer 的补充，以提供在命令行上进行搜索的能力。代码清单 1.2 展示 Searcher 程序完整代码。程序需要两个命令行参数：
- 由 Indexer 创建的索引库路径
- 用于搜索该索引库的查询
