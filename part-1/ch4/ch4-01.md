...
&emsp;&emsp;下一步，使用同样的四个分析器对语句 "XY&Z Corporation - xyz@example.com" 进行分析：
...
IndexWriterConfig 实例一经创建，分析器就不能再修改了。IndexWriter 也是如此，实例一经创建，其 IndexWriterConfig 就不能修改，因此也就无法修改分析器。
